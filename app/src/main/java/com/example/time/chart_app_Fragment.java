...
public class chart_app_Fragment extends Fragment {  //chart界面表格
    public chart_app_Fragment(){
    }
    ...
    public View onCreateView(LayoutInflater inflater,ViewGroup container,
        ...
        list=fragmentCallBack.callbackFun2();
        for(int i=0;i<list.size();i++){
            name.add(list.get(i).getApp());
            String str=list.get(i).getTime()+ "";
            time.add(str);
            img.add(list.get(i).getIcon());
        }
    ...
    private List<App> getApp() {
        ...
        for (int i = 0; i <j; i++) {//i<count:软件数目
            App[] app = new App[j];
//            app[i] = new App("应用名称", R.drawable.ic_launcher_background, "20");
